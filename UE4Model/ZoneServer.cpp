...
void ZoneServer::HandleConfirmRequest(const shared_ptr<UE4Client>& client, NioInPacket& in_packet)
{
    ...
    std::shared_ptr<Character> chr = Character::Create(client->GetAccid(), client->GetCid());
    bool result{ false };
    try {
...
void ZoneServer::HandleChrPositionNotify(const shared_ptr<UE4Client>& client, NioInPacket& in_packet)
{
